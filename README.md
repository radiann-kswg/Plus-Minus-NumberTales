...
| 要素 | 中身 |
| --- | --- |
| 目標の数字 | キャラ選択で決まる（1〜9）。器の数字が **±目標** になると得点（200）、左右で **＋目標と−目標** が同時に揃うと**全消し**（2000） |
...
| レベル | スコア 5000 ごとに 1 上昇（上限 51 で `MAX`）。上がるほどベルトが速く、出る数字の幅が広がる |
...
| ビルド | **WebGL**（unityroom 向け）。エディタでは `TitleScene` から Play |
...
Unity Hub で開き、`TitleScene` を Play。WebGL ビルドは Build Profiles から。
